...
import src.misc.neuron_lif as neuron_lif


class AbstractLIF(torch.nn.Module):
    """Abstract block class for Current Based Leaky Integrator neuron. This
    should never be instantiated on it's own.
    """
    def __init__(self, *args, **kwargs):
        super(AbstractLIF, self).__init__(*args, **kwargs)
        if self.neuron_params is not None:
            self.neuron = neuron_lif.Neuron(**self.neuron_params)
...
class Dense(AbstractLIF, AbstractDense):
    def __init__(self, *args, **kwargs):
...
class Conv(AbstractLIF, AbstractConv):
    def __init__(self, *args, **kwargs):
