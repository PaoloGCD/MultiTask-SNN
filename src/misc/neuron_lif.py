...
from lava.lib.dl.slayer.spike import Spike

import src.misc.base_multitask as base_multitask

SCALE_RHO_MULT = 0.1
TAU_RHO_MULT = 100


class Neuron(base_multitask.Neuron):
    ...
    def dynamics(self, input):
        ...
        if self.shape is None:
            ...
            # TODO Paolo - how instantiate this?
            if self.norm is not None:
                self.norm = self.norm(self.shape[0]).to(self.device)
        ...
        voltage = leaky_integrator.dynamics(
            input+self.bias,  # bias can be enabled by adding it here
