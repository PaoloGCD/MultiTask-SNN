...
from src.misc.dataset_cifar10dvs import CIFAR10DVSDataset, augment
...
result_path = f'results/CIFAR10DVS-vgg-{experiment_number:02d}'
...
class Network(torch.nn.Module):
    def __init__(self):
        ...
        neuron_params_norm_drop = {**neuron_params, 'dropout': slayer.neuron.Dropout(p=0.5), 'norm': torch.nn.BatchNorm3d }

        self.blocks = torch.nn.ModuleList([
            # slayer.block.cuba.Conv(neuron_batch_norm, 2, 128, 3, weight_norm=weight_norm, delay=False, padding=1),
            # slayer.block.cuba.Pool(neuron_params, 2, weight_norm=weight_norm, delay=False),
            # slayer.block.cuba.Conv(neuron_batch_norm, 128, 128, 3, weight_norm=weight_norm, delay=False, padding=1),
            # slayer.block.cuba.Pool(neuron_params, 2, weight_norm=weight_norm, delay=False),
            # slayer.block.cuba.Conv(neuron_batch_norm, 128, 128, 3, weight_norm=weight_norm, delay=False, padding=1),
            # slayer.block.cuba.Pool(neuron_params, 2, weight_norm=weight_norm, delay=False),
            # slayer.block.cuba.Conv(neuron_batch_norm, 128, 128, 3, weight_norm=weight_norm, delay=False, padding=1),
            # slayer.block.cuba.Pool(neuron_params_drop, 2, weight_norm=weight_norm, delay=False),
            ...
            # slayer.block.cuba.Conv(neuron_batch_norm, 128, 128, 3, weight_norm=weight_norm, delay=False, padding=1),
            # torch.nn.MaxPool3d((2, 2, 1), stride=(2, 2, 1)),
            # slayer.block.cuba.Conv(neuron_params_norm_drop, 128, 128, 3, weight_norm=weight_norm, delay=False, padding=1),
            # torch.nn.MaxPool3d((2, 2, 1), stride=(2, 2, 1)),
            # slayer.block.cuba.Conv(neuron_params_norm_drop, 128, 128, 3, weight_norm=weight_norm, delay=False, padding=1),
            # torch.nn.MaxPool3d((2, 2, 1), stride=(2, 2, 1)),
            # slayer.block.cuba.Flatten(),

            # slayer.block.cuba.Dense(neuron_params_drop, 8192, 2048, weight_scale=30, weight_norm=weight_norm, delay=False),
            # slayer.block.cuba.Dense(neuron_params_drop, 2048, 256, weight_scale=30, weight_norm=weight_norm, delay=False),
            # slayer.block.cuba.Dense(neuron_params, 256, 10, weight_scale=30, weight_norm=weight_norm),
            ...
            torch.nn.MaxPool3d((2, 2, 1), stride=(2, 2, 1)),
            slayer.block.cuba.Flatten(),
            lif_multitask.Dense(neuron_params_drop, 8192, 2048, weight_scale=30, weight_norm=weight_norm, delay=False),
            lif_multitask.Dense(neuron_params_drop, 2048, 100, weight_scale=30, weight_norm=weight_norm, delay=False),
    ...
    def print_neuron_current_decay(self):
        ...
        for block in self.blocks:
            if hasattr(block, 'neuron'):
                print("%6.4f" % block.neuron.current_decay.grad.item() if block.neuron.current_decay.grad is not None else 0.0, end=' ')
        print(']')
    ...
    def print_neuron_voltage_decay(self):
        ...
        for block in self.blocks:
            if hasattr(block, 'neuron'):
                print("%6.4f" % block.neuron.voltage_decay.grad.item() if block.neuron.voltage_decay.grad is not None else 0.0, end=' ')
        print(']')
...
training_set = CIFAR10DVSDataset(train=True,
                                 path=data_path,
                                 ...
                                 keep_value=True,
                                 transform=augment)
testing_set = CIFAR10DVSDataset(train=False,
                                path=data_path,
                                sub_path='/20frames128sizeTimeValue',
                                time_steps=20,
                                output_size=128,
                                codification='time',
                                keep_value=True)
